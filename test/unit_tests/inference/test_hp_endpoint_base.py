...
class TestHPEndpointBase(unittest.TestCase):
    ...
    @patch("sagemaker.hyperpod.common.utils.validate_cluster_connection", return_value=True)
    ...
    def test_call_list_api(self, mock_validate_connection, mock_custom_api):
        mock_custom_api.return_value.list_namespaced_custom_object.return_value = {"items": []}
    ...
    def test_call_get_api(self, mock_validate_connection, mock_custom_api):
        mock_custom_api.return_value.get_namespaced_custom_object.return_value = {"name": "test"}
    ...
    def test_get_operator_logs(self, mock_validate_connection, mock_core_api):
        ...
        mock_core_api.return_value.read_namespaced_pod_log.assert_called_once_with(
            name="test-pod", namespace=OPERATOR_NAMESPACE, timestamps=True, since_seconds=7200
        )
    ...
    def test_get_logs(self, mock_validate_connection, mock_core_api):
        ...
        mock_core_api.return_value.read_namespaced_pod_log.assert_called_once_with(
            name="test-pod", namespace="test-ns", container="test-container", timestamps=True
        )