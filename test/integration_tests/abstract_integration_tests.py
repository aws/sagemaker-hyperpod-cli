...
class AbstractIntegrationTests():
        ...
        if describe:
            ...
            if cfn_output and cfn_output.get("Outputs"):
                for output in cfn_output.get("Outputs"):
                    self.cfn_output_map[output.get("OutputKey")] = output.get("OutputValue")

        # Check if S3 bucket stack fro life cycle script exists else we will create it
        # We do not clean up this bucket with every test run
        try:
            self.describe_s3_satck_and_set_values(cfn)
        except ClientError as e:
            logger.info(f"Got error while describing s3 stack {e}, Creating stack again")
            with open("test/integration_tests/cloudformation/s3.yaml", 'r') as fh:
                template = fh.read()
                cfn.create_stack(
    ...
    def upload_lifecycle_script(self, session):
        ...
        try:
            response = s3_client.upload_file("test/integration_tests/lifecycle_script/on_create_noop.sh",
                                             self.cfn_output_map.get("Bucket"), "on_create_noop.sh")
        except ClientError as e:
    ...
    def create_hyperpod_cluster(self, session):
        ...
        sagemaker_client.create_cluster(
            ClusterName=self.hyperpod_cli_cluster_name,
            Orchestrator={'Eks': {
                'ClusterArn': self.cfn_output_map.get("ClusterArn")}
            },
            InstanceGroups=[
                {
            ...
            VpcConfig={
                "SecurityGroupIds": [
                    self.cfn_output_map.get("NoIngressSecurityGroup")
                ],
                "Subnets": [
                    self.cfn_output_map.get("PrivateSubnet1")
                ]
            }
        )
        ...
        try:
            ...
            while result.get("ClusterStatus") not in self.hyperpod_cluster_terminal_state:
                time.sleep(30)
                result = self.get_hyperpod_cluster_status(sagemaker_client)
    ...
    def delete_hyperpod_cluster(self, session):
        ...
        # TODO: removing dev endpoint
        sagemaker_client = session.client("sagemaker")
        sagemaker_client.delete_cluster(
            ClusterName=self.hyperpod_cli_cluster_name
        )
        ...
        try:
            ...
            logger.info(f"Caught exception while trying to describe cluster during teardown {e}")
            return
        raise Exception(f"Hyperpod Cluster {self.hyperpod_cli_cluster_name} fail to delete")
    ...
    def describe_s3_satck_and_set_values(self, cfn_client):
        describe_s3_stack = cfn_client.describe_stacks(StackName=self.s3_roles_stack_name)
    ...
    def update_cluster_auth(self):
        with open("test/integration_tests/charts/hp-node-auth.yaml", "r") as hyperpod_current_context:
            template =  hyperpod_current_context.read()

        template = re.sub("SAGEMAKER_EXECUTION_ROLE", self.cfn_output_map.get("ExecutionRole"), template)
        template = re.sub("SAGEMAKER_SERVICE_ROLE", self.cfn_output_map.get("ServiceRole"), template)
        ...
        try:
            # Execute the command to update kubeconfig
            logger.info(subprocess.run(command, check=True, capture_output=True, text=True))
        except subprocess.CalledProcessError as e:
