...
USAGE_GUIDE_TEXT = """# SageMaker HyperPod CLI - Initialization Workflow
...
You can provide configuration values during initialization:
...
The `submit` command processes your configuration and creates the endpoint. It injects values from `config.yaml` into the `k8s.jinja` template and creates a timestamped record in the `runs` directory.
...
1. Initialize a new endpoint configuration:
   ```bash
   hyp init hyp-jumpstart-endpoint my-endpoint
   cd my-endpoint
   ```
...
4. Submit the endpoint creation request:
   ```bash
   hyp submit
   ```
