...
def _validate_namespace_argument(config: DictConfig) -> None:
    ...
    if namespace_argument is not None and not re.match(namespace_regex, namespace_argument):
        raise ValueError("Provided namespace is not valid, Kindly provide Kubernetes Namespace "
...
def _validate_k8_custom_script_workflow_mandatory_argument(config: DictConfig) -> None:
    ...
    if cluster_type == "k8s" and training_cfg is not None:
        k8_custom_script_mandatory_arguments = ["container", "env_vars",
                                                "training_cfg", "training_cfg.entry_script",
                                                "training_cfg.run", "training_cfg.run.name", "training_cfg.run.nodes",
                                                "cluster", "cluster.cluster_type", "cluster.instance_type", "cluster.cluster_config"]
